...
def _merge_pdfs(pdf1_path, pdf2_path, output_path):
    try:
        ...
        logger.warning(f"_merge_pdfs_ng failed, fallback to legacy: {e_ng}")
        logger.info("Merging PDFs using _merge_pdfs_legacy")
        _merge_pdfs_legacy(pdf1_path, pdf2_path, output_path)


def _merge_page_with_translate(new_page, src_page, tx, ty):
    import PyPDF2
    ...
    if tx or ty:
        page_copy.add_transformation(PyPDF2.Transformation().translate(tx, ty))

    try:
        new_page.merge_page(page_copy, expand=False)
    except TypeError:
    ...
    with open(pdf1_path, "rb") as pdf1_file:
        ...
        with open(pdf2_path, "rb") as pdf2_file:
            ...
            for page_num in range(num_pages):
                if page_num < len(pdf1_reader.pages):
                    page1 = pdf1_reader.pages[page_num]
                else:
                    page1 = PyPDF2.PageObject.create_blank_page(width=612, height=792)

                if page_num < len(pdf2_reader.pages):
                    page2 = pdf2_reader.pages[page_num]
                else:
                    page2 = PyPDF2.PageObject.create_blank_page(width=612, height=792)
                ...
                new_page = PyPDF2.PageObject.create_blank_page(
                    width=int(width1 + width2 * percent),
                    height=height,
                )

                _merge_page_with_translate(new_page, page1, 0, 0)
                _merge_page_with_translate(new_page, page2, width1, 0)
    ...
    with open(pdf1_path, "rb") as pdf1_file:
        ...
        with open(pdf2_path, "rb") as pdf2_file:
            ...
            for page_num in range(num_pages):
                ...
                _merge_page_with_translate(new_page, page1, 0, 0)
                _merge_page_with_translate(new_page, page2, width1 - width2 * (1 - percent), 0)
                output_writer.add_page(new_page)
...
# Call directly for better Windows compatibility.
merge_pdfs = _merge_pdfs
